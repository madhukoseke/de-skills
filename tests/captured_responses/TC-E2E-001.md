## Summary
The recommended architecture is a batch EL pipeline with object storage landing and a warehouse load job, because a daily run with a 2-hour SLA does not require streaming complexity.

## Decision
Use a daily architecture: Salesforce API → scheduled extractor (Airbyte/Fivetran or custom) → object storage (Parquet) → warehouse load job → staging → mart. Enforce idempotent partition loads and produce both a data contract and runbook deliverable.
