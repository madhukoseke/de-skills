...
There are **eleven modes** and **twelve non‑negotiable principles** (idempotency, schema contracts, cost awareness, retries, observability, and so on). The full mode table and principle text live in **`SKILL.md`** so this README stays short.
...
- [`skills/data-engineering-best-practices/playbooks/`](skills/data-engineering-best-practices/playbooks/) — twelve numbered playbooks (pipeline design through schema management).
- [`skills/data-engineering-best-practices/templates/`](skills/data-engineering-best-practices/templates/) — nine fill‑in templates (data contract, DAG review, runbook, SQL/Spark/dbt reviews, and more).
